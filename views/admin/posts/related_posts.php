<?php
include 'path.php';
$isEditting = isset($this->post['slug']);
$relatedPostIds = array_column($this->relatedPosts, 'id');
$allPosts = \App\Utils::removePostBodyColumn($this->posts);
$relatedPosts = \App\Utils::removePostBodyColumn($this->relatedPosts);
...
<head>
  ...
  <title>Select Related Posts - Admin</title>
</head>
...
<body>
      ...
      <div class="admin-container">
        <div class="table-container double-column">
          ...
          <div class="sm-box">
              ...
              <tbody>
                <?php if (!empty($allPosts)): ?>
                  <?php foreach ($allPosts as $key => $post): ?>
                    <tr
                      class="post-tr-<?php echo $post['id']; ?> <?php echo in_array($post['id'], $relatedPostIds) ? 'selected' : ''; ?>"
            ...
            <textarea name="related-posts" id="related-posts" class="related-posts hide">
              <?php echo json_encode($relatedPosts); ?>
            </textarea>
          ...
          <div class="sm-box">
            <form action="<?php url("/admin/posts/related-posts/" . $this->post['id']); ?>" method="post">
              <h2>Related posts</h2>
              ...
              <table class="selected-posts-table">
                <thead>
                  <tr>
                    <th colspan="2">
                      <a href="<?php url("/posts/" . $this->post['slug']); ?>" target="_blank" class="link">
                ...
                <tbody>
                  <?php if (!empty($this->relatedPosts)): ?>
                    <?php foreach ($this->relatedPosts as $key => $post): ?>
                      <tr
                      ...
                      >
                        ...
                        <td class="title">
                          <?php echo $post['title']; ?>
                          <input type="hidden" name="relatedPostIds[]" value="<?php echo $post['id']; ?>">
                          <input type="hidden" class="sort_order_input" name="sort_order[]" value="<?php echo $post['sort_order']; ?>">
                        </td>
                        <td style="width: 50px;">
                          <ion-icon data-id="<?php echo $post['id'] ?>" class="hide close-icon" name="close-outline"></ion-icon>
                        </td>
                      </tr>
                    <?php endforeach;?>
                  <?php else: ?>
                ...
                <tfoot class="table-pagination">
                  <td colspan="3">
                    <button type="submit" class="btn primary-btn long-btn">Save Related Posts</button>
                  </td>
                </tfoot>
              </table>
            </form>
  ...
  <script>
    ...
    const selectedPosts = JSON.parse(document.querySelector('.related-posts').value) || [];
        ...
        postRows = `${postRows}
          <tr class="post-tr-${post.id} selected" onclick="unSelectPost(this)" data-id="${post.id}" data-title="${post.title}">
            <td class="sort-order">
              ...
              <td class="title">
              ${post.title}
              <input type="hidden" name="relatedPostIds[]" value="${post.id}" />
